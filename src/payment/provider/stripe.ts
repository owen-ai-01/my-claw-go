...
export class StripeProvider implements PaymentProvider {
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes('No such price')) {
          throw new Error(
            'Invalid price configuration. Please contact support.'
          );
        }
        ...
        if (error.message.includes('rate_limit')) {
          throw new Error(
            'Payment system is temporarily busy. Please try again.'
          );
        }
      }
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('No configuration provided')) {
          throw new Error(
            'Customer portal is not configured. Please contact support to set up billing portal.'
          );
        }
      }

      throw new Error(
        'Failed to create customer portal. Please try again or contact support.'
      );
    }
  }
  ...
  public async handleWebhookEvent(
    ...
    try {
      ...
      console.log(`handle webhook event, type: ${eventType}, id: ${event.id}`);
  ...
  private async onDeleteSubscription(
    ...
    if (result.length > 0) {
      ...
      if (userId) {
        // Only revoke keys and stop VPS when the subscription actually expired
        // at period end. Immediate cancellations still have current_period_end
        // in the future — the user paid for that time and should keep full access.
        ...
        if (periodEndMs <= Date.now() + 300_000) {
          revokeUserOpenrouterKey(userId).catch((e) =>
            console.error('[OR-Key] revoke error after subscription expired:', e)
          );

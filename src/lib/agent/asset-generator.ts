...
import type { GeneratedAssets, Storyboard } from './url-processor';
...
export class AssetGenerator {
    ...
    try {
      ...
      for (const scene of scenes) {
        if (scene.visualType === 'screenshot') {
          ...
          if (
            !scene.visualPrompt ||
            scene.visualPrompt.includes('screenshot')
          ) {
            scene.visualPrompt = `Professional website interface mockup, modern UI design, clean layout, ${scene.description}`;
          }
          console.log(
            `[AssetGenerator] Converted scene ${scene.id} from screenshot to AI image`
          );
        }
      }
      ...
      for (const scene of scenes) {
        try {
          ...
          if (imageUrl && typeof imageUrl === 'string') {
            ...
            console.error(
              `Invalid imageUrl type for scene ${scene.id}:`,
              typeof imageUrl,
              imageUrl
            );
          }
          ...
          console.error(
            `Failed to process asset for scene ${scene.id}:`,
            error
          );
          scene.assetUrl = `https://placehold.co/1920x1080/png?text=Scene+${scene.sceneNumber}`;
        }
  ...
  private async uploadAsset(
    ...
    try {
      const base64Data = assetData.includes(',')
        ? assetData.split(',')[1]
        : assetData;

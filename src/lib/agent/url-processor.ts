...
export class UrlToVideoAgent {
  ...
  async process(): Promise<AgentState> {
    try {
      ...
      const scrapedData = await scrapeProductUrl(this.state.url, {
      ...
      this.log(
        `Extracted ${scrapedData.images.length} images and ${scrapedData.description.length} chars of text.`
      );
      ...
      if (storyboard) {
        this.log(
          `✅ Using cached storyboard for ${this.state.url} (AI Credits saved)`
        );
      } else {
      ...
      this.log(
        `Storyboard created: "${storyboard.title}" with ${storyboard.scenes.length} scenes.`
      );
  ...
  private async analyzeContent(scrapedData: any): Promise<ContentAnalysis> {
    ...
    const contentToAnalyze =
      scrapedData.content || JSON.stringify(scrapedData).slice(0, 5000);

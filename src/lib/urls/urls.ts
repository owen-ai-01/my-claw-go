...
export function getUrlWithLocaleInCallbackUrl(
  ...
  try {
    ...
    if (callbackURL) {
      // Check if callbackURL is a full URL or a relative path
      try {
        const callbackUrlObj = new URL(callbackURL);
        // If it's a full URL, check if the pathname already includes the locale
        if (!callbackUrlObj.pathname.match(new RegExp(`^/${locale}(/|$)`))) {
          // Add locale to the pathname
          const localizedPathname = callbackUrlObj.pathname.startsWith('/')
            ? `/${locale}${callbackUrlObj.pathname}`
            : `/${locale}/${callbackUrlObj.pathname}`;

          callbackUrlObj.pathname = localizedPathname;
          urlObj.searchParams.set('callbackURL', callbackUrlObj.toString());
        }
      } catch {
        // If callbackURL is not a valid URL, treat it as a relative path

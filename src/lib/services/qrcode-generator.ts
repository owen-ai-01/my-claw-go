...
export class QRCodeGenerator {
    ...
    async generateDataUrl(url: string): Promise<string> {
        console.log(`[QRCode] 📱 Generating QR code for URL: ${url.substring(0, 50)}...`);
    ...
    async generateAccessibleUrl(url: string): Promise<string> {
        console.log(`[QRCode] 📱 Generating accessible QR code URL for: ${url.substring(0, 50)}...`);
        ...
        try {
            ...
            console.log(`[QRCode] Primary (data URL): ${dataUrl.substring(0, 50)}...`);
            console.log(`[QRCode] Fallback (qrserver): ${fallbackUrl.substring(0, 80)}...`);

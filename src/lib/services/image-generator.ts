...
export async function generateSceneImage(
  ...
  prompt +=
    ', professional, high quality, 16:9 aspect ratio, 4k, clean composition';
  ...
  try {
    ...
    console.log(
      `[DEBUG] Created prediction ${prediction.id}, status: ${prediction.status}`
    );
    ...
    while (
      ...
      console.log(
        `[DEBUG] Prediction ${prediction.id} status: ${finalPrediction.status}`
      );
    }

    if (finalPrediction.status === 'failed') {
      throw new Error(
        `Prediction failed: ${finalPrediction.error || 'Unknown error'}`
      );
    }

    console.log(
      `[DEBUG] Prediction succeeded, output:`,
...
export async function generateImagesForScenes(
  ...
  for (let i = 0; i < aiScenes.length; i += batchSize) {
    ...
    console.log(
      `Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(aiScenes.length / batchSize)}`
    );
    ...
    results.forEach((result, index) => {
      ...
      if (result.status === 'fulfilled') {
        ...
        console.error(
          `❌ Failed to generate image for ${scene.id}:`,
          result.reason
        );
        // Use placeholder

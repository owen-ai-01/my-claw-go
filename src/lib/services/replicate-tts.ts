...
export class ReplicateTTSService {
     ...
     */
    async generateTTS(text: string, voice: string = 'af_bella'): Promise<string> {
        console.log(`[Replicate TTS] 🎤 Generating TTS for: "${text.slice(0, 50)}..."`);
        console.log(`[Replicate TTS] Voice: ${voice}, Text length: ${text.length} chars`);
        ...
        try {
            ...
            console.log(`[Replicate TTS] Prediction ID: ${prediction.id}, Status: ${prediction.status}`);
            ...
            while (result.status !== 'succeeded' && result.status !== 'failed' && attempts < maxAttempts) {
                ...
                attempts++;
                console.log(`[Replicate TTS] Polling attempt ${attempts}: ${result.status}`);
            }

            if (result.status === 'failed') {
                throw new Error(`TTS prediction failed: ${result.error || 'Unknown error'}`);
            }
            ...
            console.log(`[Replicate TTS] Raw output:`, JSON.stringify(result.output, null, 2));
            ...
            if (typeof output === 'string') {
                ...
                // 数组格式
                    ...
                    if (typeof firstItem === 'string') {
                        ...
                        audioUrl = typeof firstItem.url === 'function' ? await firstItem.url() : firstItem.url;
                    }
                }
            }

            if (!audioUrl || typeof audioUrl !== 'string') {
                throw new Error(`No valid audio URL in prediction output: ${JSON.stringify(output)}`);
            }

            console.log(`[Replicate TTS] 📥 Got audio URL: ${audioUrl.substring(0, 60)}...`);
     ...
     * @returns 音频 URL 数组
     */
    async generateBatchTTS(texts: string[], voice: string = 'af_bella'): Promise<(string | null)[]> {

...
export class ShotstackVideoComposer {
     ...
     * @returns 生成的音频 URL
     */
    async generateTTS(text: string, voice: string = 'Matthew'): Promise<string> {
        console.log(`[Shotstack TTS] 🎤 Generating TTS for: "${text.slice(0, 50)}..."`);
        ...
        for (let retry = 0; retry < maxRetries; retry++) {
            try {
                if (retry > 0) {
                    console.log(`[Shotstack TTS] 🔄 Retry attempt ${retry + 1}/${maxRetries}...`);
                ...
                if (error.cause?.code === 'ECONNRESET' || error.message?.includes('fetch failed')) {
                    console.warn(`[Shotstack TTS] ⚠️ Network error, will retry: ${error.message}`);
                    continue;
                }
                // 其他错误直接抛出
                throw error;
    ...
    private async pollTTSStatus(assetId: string): Promise<string> {
        const createUrl = this.baseUrl.replace('/edit/', '/create/').replace('/stage', '/create/v1');
        let attempts = 0;
        ...
        while (attempts < maxAttempts) {
            ...
            try {
                ...
                if (!response.ok) {
                    console.warn(`[Shotstack TTS] Poll attempt ${attempts} failed: ${response.status}`);
                    continue;
                }
    ...
    private async pollRenderStatusViaRest(renderId: string): Promise<string> {
        ...
        while (attempts < maxAttempts) {
            ...
            try {
                ...
                if (!response.ok) {
                    console.warn(`[Shotstack] Poll attempt ${attempts} failed: ${response.status}`);
                    continue;
                }
                ...
                console.log(`[Shotstack] Render status: ${status} (attempt ${attempts})`);
    ...
    private async prepareAssetUrl(url: string): Promise<string | null> {
        ...
        if (url.includes('storage.googleapis.com') && url.includes('Signature=')) {
            console.log('[Shotstack] ⚠️ Skipping Google Cloud Storage signed URL (Shotstack incompatible)');
    ...
    async composeVideo(scenes: Scene[], sourceUrl?: string): Promise<string> {
        console.log(`[Shotstack] 🎬 Composing video with ${scenes.length} scenes...`);
        if (sourceUrl) {
            console.log(`[Shotstack] 📱 Will add URL and QR code to final scene: ${sourceUrl.substring(0, 50)}...`);
        }
        ...
        try {
            ...
            const validScenes = scenes.filter(scene => {
                if (!scene.assetUrl || typeof scene.assetUrl !== 'string') {
                    console.warn(`[Shotstack] ⚠️ Scene ${scene.id} has no valid assetUrl, skipping`);
                    return false;
                }
                if (!scene.duration || scene.duration <= 0) {
                    console.warn(`[Shotstack] ⚠️ Scene ${scene.id} has invalid duration (${scene.duration}), using default 5s`);
                    scene.duration = 5;
                }
                // 验证 URL 格式
                try {
                    ...
                    console.warn(`[Shotstack] ⚠️ Scene ${scene.id} has invalid URL format: ${scene.assetUrl}, skipping`);
                    return false;
                }
                return true;
            });
            ...
            console.log(`[Shotstack] ✅ Validated ${validScenes.length}/${scenes.length} scenes`);
            ...
            for (const scene of validScenes) {
                ...
                if (preparedUrl) {
                    ...
                    console.log(`[Shotstack] ⏭️ Skipping scene ${scene.id} due to incompatible URL`);
                }
            }
            ...
            console.log(`[Shotstack] ✅ ${processedScenes.length}/${validScenes.length} scenes have compatible URLs`);

            // 打印场景详情便于调试
            processedScenes.forEach((scene, idx) => {
                console.log(`[Shotstack] Scene ${idx + 1}: id=${scene.id}, duration=${scene.duration}s, url=${scene.assetUrl?.substring(0, 60)}...`);
            });

            // 1. 使用 Replicate Kokoro TTS 为每个场景生成语音
            console.log('[Shotstack] 🎤 Generating TTS with Replicate Kokoro for all scenes...');
            ...
            try {
                ...
                for (const scene of processedScenes) {
                    if (scene.narration && scene.narration.trim().length > 0) {
                        try {
                            const audioUrl = await replicateTTS.generateTTS(scene.narration, 'af_bella');
                            if (audioUrl && typeof audioUrl === 'string') {
                                ttsUrls.set(scene.id, audioUrl);
                            }
                        } catch (error) {
                            console.warn(`[Shotstack] ⚠️ TTS failed for scene ${scene.id}, skipping audio:`, error);
                        }
                    }
                }
            } catch (error) {
                console.warn('[Shotstack] ⚠️ Replicate TTS service initialization failed, continuing without TTS:', error);
            }
            ...
            // 原始数据用于 REST API 回退
            const rawVideoData: Array<{ src: string; start: number; length: number; transition?: boolean }> = [];
            const rawAudioData: Array<{ src: string; start: number; length: number; volume: number }> = [];
            const rawTextData: Array<{ text: string; start: number; length: number }> = [];
            // QR码和URL叠加数据 (用于最后一个场景)
            const rawQrOverlay: Array<{ src: string; start: number; length: number }> = [];
            const rawUrlTextOverlay: Array<{ text: string; start: number; length: number }> = [];
            ...
            if (sourceUrl) {
                try {
                    ...
                    console.log(`[Shotstack] 📱 QR code URL generated: ${qrCodeUrl.substring(0, 80)}...`);
                } catch (error) {
                    console.warn('[Shotstack] ⚠️ Failed to generate QR code, continuing without it:', error);
                }
            }
            ...
            if (sourceUrl && processedScenes.length > 0) {
                ...
                // 添加URL文字叠加 (在底部显示)
                const urlDisplayText = sourceUrl.length > 50 ? sourceUrl.substring(0, 47) + '...' : sourceUrl;
            ...
            console.log(`[Shotstack] Built ${videoClips.length} video clips, ${audioClips.length} audio clips, total duration: ${currentTime}s`);

            // 3. 创建轨道（视频和音频分开）
            const tracks: Track[] = [
                new Track().setClips(videoClips),
            ];
            ...
            try {
                ...
                console.log('[Shotstack] Raw response:', JSON.stringify(renderResponse, null, 2));

                const renderId = renderResponse.response?.id || renderResponse.data?.response?.id;

                if (!renderId) {
                    console.error('[Shotstack] Invalid response structure:', JSON.stringify(renderResponse, null, 2));
                ...
                console.error('  - Body:', JSON.stringify(apiError.body || apiError.response?.body, null, 2));

                // 尝试使用 REST API 直接提交
                console.log('[Shotstack] 🔄 Trying fallback with REST API...');
                const renderId = await this.submitRenderViaRest(rawVideoData, rawAudioData, rawTextData, rawQrOverlay, rawUrlTextOverlay);
                const videoUrl = await this.pollRenderStatusViaRest(renderId);
                console.log(`[Shotstack] ✅ Video composition complete (via REST): ${videoUrl}`);
                return videoUrl;
            }
    ...
    private async submitRenderViaRest(
        videoClipData: Array<{ src: string; start: number; length: number; transition?: boolean }>,
        audioClipData: Array<{ src: string; start: number; length: number; volume: number }>,
        textClipData: Array<{ text: string; start: number; length: number }>,
        qrOverlayData: Array<{ src: string; start: number; length: number }> = [],
        urlTextOverlayData: Array<{ text: string; start: number; length: number }> = []
    ): Promise<string> {
        ...
        console.log('[Shotstack] REST Request body:', JSON.stringify(requestBody, null, 2));

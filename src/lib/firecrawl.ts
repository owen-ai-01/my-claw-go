...
function getCacheFilePath(url: string, includeScreenshot: boolean): string {
  const hash = crypto
    .createHash('md5')
    .update(url + (includeScreenshot ? '_shot' : ''))
    .digest('hex');
...
export async function scrapeProductUrl(
  ...
  if (fs.existsSync(cacheFile)) {
    try {
      ...
      console.warn(
        '[Firecrawl] Failed to read cache file, fetching fresh data.'
      );
    }
  }
  ...
  try {
    ...
    const description =
      data.description ||
      (scrapeResult as any).metadata?.description ||
      (scrapeResult.markdown ? scrapeResult.markdown.slice(0, 200) : '');

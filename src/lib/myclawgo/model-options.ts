...
import { AVAILABLE_MODELS, guessModelLabel, type ModelOption } from './model-catalog';
...
function isSelectablePricingModel(model: string): boolean {
  ...
  if (model === 'openrouter/auto' || model === 'openrouter/openrouter/auto') return false;
  return true;
}

export function getSelectableModelOptions(): ModelOption[] {
  const pricingModels = new Set(getPricingModelKeys().filter(isSelectablePricingModel));
  ...
  const defaults = AVAILABLE_MODELS
    .filter((m) => pricingModels.has(m.id))

...
function getL1Model(): string {
  return (
    process.env.MYCLAWGO_ROUTER_L1_MODEL ||
...
function getL2Model(intent: 'chinese' | 'code' | 'general'): string {
  if (intent === 'chinese')
    return (
      process.env.MYCLAWGO_ROUTER_L2_ZH_MODEL ||
      'openrouter/deepseek/deepseek-v3'
    );
  if (intent === 'code')
    return (
      process.env.MYCLAWGO_ROUTER_L2_CODE_MODEL ||
      'openrouter/anthropic/claude-haiku-4.5'
    );
  return (
    process.env.MYCLAWGO_ROUTER_L2_MODEL ||
    'openrouter/anthropic/claude-haiku-4.5'
  );
}
function getL3Model(intent: 'longContext' | 'general'): string {
  if (intent === 'longContext')
    return (
      process.env.MYCLAWGO_ROUTER_L3_LONG_MODEL ||
      'openrouter/google/gemini-2.5-pro'
    );
  return (
    process.env.MYCLAWGO_ROUTER_L3_MODEL ||
...
const CODE_KEYWORDS =
  /```|function |def |class |import |require\(|async |await |error:|exception|traceback|syntax error|bug|fix|debug|refactor|代码|报错|调试|修复|接口|API|SQL|query|shell|bash|python|javascript|typescript|rust|golang|java/i;
const ARCHITECTURE_KEYWORDS =
  /架构|系统设计|技术方案|模块|高并发|分布式|微服务|database design|system design|architecture|design pattern|PRD|需求文档|方案设计|技术选型/i;
const CHINESE_WRITING_KEYWORDS =
  /写一篇|帮我写|润色|翻译|博客|文章|小红书|公众号|朋友圈|营销文案|copywriting|改写|扩写|缩写|总结一下.*文章|summarize/i;
const TOOL_KEYWORDS =
  /帮我搜|搜索一下|查一下|浏览器|打开网页|browse|search the web|look up|find me/i;
...
export function routeMessage(params: {
  ...
  if (CHINESE_WRITING_KEYWORDS.test(msg)) {
    return {
      level: 'L2',
      path: 'bridge',
      model: getL2Model('chinese'),
      reason: 'chinese_writing',
    };
  }

  // 9. L2: Message length medium
  if (len > LONG_CONTEXT_THRESHOLD) {
    const model = isMostlyChinese(msg)
      ? getL2Model('chinese')
      : getL2Model('general');

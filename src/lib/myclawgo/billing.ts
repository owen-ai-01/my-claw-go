 ...
 */
function resolveModelKeyWithKey(
  model: string,
  ...
  if (withoutProvider && pricingMap[withoutProvider])
    return { key: withoutProvider, pricing: pricingMap[withoutProvider]! };
  // 3. Try with common provider prefixes
  for (const prefix of ['openrouter', 'openai', 'anthropic', 'google']) {
    if (pricingMap[`${prefix}/${model}`])
      return {
        key: `${prefix}/${model}`,
        pricing: pricingMap[`${prefix}/${model}`]!,
      };
    // e.g. "minimax/minimax-m2.5" → "openrouter/minimax/minimax-m2.5"
    if (pricingMap[`${prefix}/${withoutProvider}`])
      return {
        key: `${prefix}/${withoutProvider}`,
        pricing: pricingMap[`${prefix}/${withoutProvider}`]!,
      };
  }
  return null;
}
...
export function calcUsdCostFromBridgeUsage(params: {
  ...
  const { inputTokens, outputTokens, cacheReadTokens } = normalizeBridgeUsage(
    params.usage
  );

...
export async function getTemplate<T extends EmailTemplate>({
  ...
  // Get the subject from the messages (fallback to empty string if not available)
  const mailMessages = (
    messages as Messages & {
      Mail?: Record<string, { subject?: string }>;
    }
  ).Mail;

  const subject = mailMessages?.[template]?.subject ?? '';

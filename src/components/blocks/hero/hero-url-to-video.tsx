...
import { Loader2, CheckCircle2, AlertCircle, Play, FileVideo, Image as ImageIcon, Download, Monitor, Clock, Settings2, Sparkles } from "lucide-react";
...
export default function HeroUrlToVideo() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (credits < VIDEO_COST) {
            toast.error(t('form.messages.insufficientCredits', { cost: VIDEO_COST, credits }));
            router.push('/pricing');
            return;
        }
    ...
    return (
        <main id="hero" className="relative min-h-[800px] flex justify-center overflow-hidden pt-4">
            {/* Background Video */}
                    ...
                    <div className="bg-background/40 backdrop-blur-md border border-white/10 rounded-2xl p-4 shadow-2xl ring-1 ring-white/10 dark:ring-white/5">
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div className="relative w-full bg-background/50 rounded-lg border border-white/10 p-6 min-h-[100px] flex flex-col justify-center">
                                <Label className="mb-2 text-muted-foreground">URL to Video - Enter any webpage URL</Label>
                                <Input
                                ...
                                <p className="text-xs text-muted-foreground mt-2">
                                    Enter a product URL, article, or any webpage. Our URL to Video AI will automatically create an engaging video for you.
                                </p>
                            </div>
                                ...
                                <div className="flex justify-end pt-2">
                                    ...
                                    >
                                        ...
                                        <span className="flex flex-col items-start leading-tight">
                                            <span>{isLoading ? t('form.generating') : t('form.generate')}</span>
                                ...
                                <CardContent className="space-y-3">
                                    {steps.map((step, index) => {
                                        ...
                                        return (
                                            <div key={step.key} className="flex items-center gap-3">
                                                <div className={`h-7 w-7 rounded-full flex items-center justify-center border text-sm ${isCompleted ? 'bg-primary text-primary-foreground border-primary' :
                                                ...
                                                <span className={`text-sm ${isCurrent ? 'font-medium text-primary' : isCompleted ? 'text-primary' : 'text-muted-foreground'}`}>
                                                    {step.label}
                                                </span>
                                            </div>
                                        );
                                    ...
                                    {state.status === 'failed' && (
                                        <div className="flex items-center gap-3 text-destructive">
                                            ...
                                            <span className="font-medium text-sm">URL to Video Failed</span>
                                        </div>
                                    )}
                                    {state.status === 'completed' && (
                                        <div className="flex items-center gap-3 text-green-600">
                                            <CheckCircle2 className="h-5 w-5" />
                                            <span className="font-medium text-sm">URL to Video Complete!</span>
                                        </div>
                                    )}
                                </CardContent>
                            </Card>
                                ...
                                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                                    <Card className="bg-background/80 backdrop-blur-sm">
                                        <CardHeader className="py-3">
                                            ...
                                            <CardDescription className="text-sm">{state.analysis.summary}</CardDescription>
                                        </CardHeader>
                                        <CardContent className="space-y-4">
                                            <div className="flex flex-wrap gap-2">
                                                <Badge variant="outline" className="text-xs">Tone: {state.analysis.tone}</Badge>
                                                <Badge variant="outline" className="text-xs">Audience: {state.analysis.targetAudience}</Badge>
                                            </div>
                                            <div className="p-3 bg-muted/30 rounded-lg border">
                                                <h4 className="text-xs font-semibold mb-1 flex items-center gap-2">
                                                    <Sparkles className="h-3 w-3 text-primary" /> Value Proposition
                                                </h4>
                                ...
                                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                                    <Card className="bg-background/80 backdrop-blur-sm">
                                        <CardHeader className="py-3">
                                            ...
                                            <CardDescription className="text-sm">{state.storyboard?.title}</CardDescription>
                                        </CardHeader>
                                        <CardContent>
                                            <div className="grid grid-cols-2 gap-3">
                                                {(state.assets?.scenes || state.storyboard?.scenes)?.map((scene: Scene) => (
                                                    <div key={scene.id} className="group relative border rounded-lg overflow-hidden bg-background">
                                                        <div className="aspect-video bg-muted relative">
                                                            {scene.assetUrl ? (
                                                                ...
                                                                <img
                                                                    ...
                                                                    onError={(e) => {
                                                                        (e.target as HTMLImageElement).src = `https://placehold.co/1920x1080/png?text=Scene+${scene.sceneNumber}`;
                                                                    }}
                                                                />
                                                            ) : (
                                                        ...
                                                        <div className="p-2">
                                                            <p className="text-[10px] font-medium line-clamp-2">{scene.narration}</p>
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                                ...
                                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
                                    <Card className="border-green-500/50 bg-green-500/5">
                                        <CardHeader className="py-3">
                                            ...
                                            <CardDescription className="text-sm">Your URL to Video has been successfully generated.</CardDescription>
                                        </CardHeader>
                                        <CardContent className="space-y-4">
                                            ...
                                            <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                                                <p className="text-xs text-amber-700 dark:text-amber-400">
                                                    ⚠️ <strong>Note:</strong> Videos are temporary and will be deleted after 24 hours. Download your URL to Video now!
                                                </p>
                                            </div>
                                        </CardContent>
                                    </Card>

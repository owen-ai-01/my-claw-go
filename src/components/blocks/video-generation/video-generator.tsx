...
export default function VideoGenerator({
  ...
  isStudioMode = false,
}: { defaultMode?: string; isStudioMode?: boolean }) {
  ...
  const [aspectRatio, setAspectRatio] = useState(
    searchParams.get('ar') || '16:9'
  );
  const [resolution, setResolution] = useState(
    searchParams.get('res') || '720p'
  );
  ...
  useEffect(() => {
    if (!prompt && activeTab === 'text-to-video') {
      const randomPrompt =
        GERMAN_VIDEO_PROMPTS[
          Math.floor(Math.random() * GERMAN_VIDEO_PROMPTS.length)
        ];
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : t('form.messages.analyzeUrlError')
      );
    } finally {
  ...
  const calculateCost = useCallback(
    (durationStr: string, resolutionStr: string) => {
      const baseCostPerSec = 10;
      const durationSec = Number.parseInt(durationStr) || 5;
    ...
    try {
      ...
      const pollInterval = setInterval(async () => {
        ...
        if (statusData.status === 'succeeded') {
          ...
          setIsGenerating(false);
          toast.error(
            `${t('form.messages.generationFailed')}: ${statusData.status}`
          );
        }
      ...
      setIsGenerating(false);
      toast.error(
        error instanceof Error ? error.message : t('form.messages.genericError')
      );
    }
  }, [
    prompt,
  ...
  useEffect(() => {
    ...
    const shouldAutoGenerate =
      sessionStorage.getItem('url_to_video_auto_generate') === 'true';
  ...
  return (
    <div
      className={cn(
        ...
        isStudioMode
          ? 'h-full p-6'
          : 'max-w-7xl mx-auto px-4 py-8 min-h-[calc(100vh-100px)]'
      )}
    >
        ...
        >
                  ...
                  <div className="relative">
                    ...
                    <div className="absolute bottom-3 left-3 z-20">
                      <Button
                        ...
                        onClick={() => {
                          const availablePrompts = GERMAN_VIDEO_PROMPTS.filter(
                            (p) => p !== prompt
                          );
                          const randomPrompt =
                            availablePrompts[
              ...
              <div className="space-y-4">
                ...
                <div>
                  <Label className="block mb-2 font-medium">
                    {t('form.promptPlaceholder')} (Optional)
                  </Label>
                  <textarea
          ...
          <div className="mt-6 pt-6 border-t border-border/50 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="resolution-select" className="text-xs">
          ...
          {activeTab !== 'web-to-video' && (
            <Button
              size="lg"
              className={cn(
                'w-full mt-8 h-12 text-lg font-medium shadow-lg transition-all',
                isGenerating
            ...
            {generatedVideo ? (
              ...
              >
                ...
                <div className="space-y-2">
                  <Label className="text-xs text-muted-foreground block pl-1">
                    {t('form.preview.generatedVideo')}
                  </Label>

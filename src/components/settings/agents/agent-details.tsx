...
function AgentTasksPanel({ agentId }: { agentId: string }) {
      ...
      if (!res.ok || data.ok !== true)
        throw new Error(
          typeof data.error === 'string'
            ? data.error
            : data.error?.message || 'Failed to load tasks'
        );
  ...
  async function toggleTask(task: TaskItem) {
    const res = await fetch(
      `/api/agents/${encodeURIComponent(agentId)}/tasks/${encodeURIComponent(task.id)}`,
      {
        method: 'PATCH',
        headers: { 'content-type': 'application/json' },
  ...
  async function runTask(taskId: string) {
    ...
    const res = await fetch(
      `/api/agents/${encodeURIComponent(agentId)}/tasks/${encodeURIComponent(taskId)}/run`,
      { method: 'POST' }
    );
    const data = (await res.json().catch(() => ({}))) as {
      ok?: boolean;
  ...
  async function loadRuns(taskId: string) {
    ...
    const res = await fetch(
      `/api/agents/${encodeURIComponent(agentId)}/tasks/${encodeURIComponent(taskId)}/runs?limit=10`,
      { cache: 'no-store' }
    );
    const data = (await res.json().catch(() => ({}))) as {
      ok?: boolean;
  ...
  return (
    <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Create and manage recurring work for this agent.
            </p>
          </div>
          <button
            type="button"
        ...
        >
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              className="mt-1 min-h-[100px] w-full rounded-xl border bg-background px-3 py-2 text-sm outline-none"
          ...
          <div className="md:col-span-2 flex justify-end">
            <button
              type="submit"
              disabled={creating || !message.trim() || !scheduleValue.trim()}
            ...
            tasks.map((task) => (
              <div key={task.id} className="rounded-xl border p-4">
                ...
                <div className="mt-3 space-y-1 text-xs text-muted-foreground">
                  <div>
                    Prompt:{' '}
                    {(task.payload?.message || task.payload?.text || '—').slice(
                      0,
                      180
                    )}
                  </div>
                  ...
                  <div>
                    Updated:{' '}
                    {formatDateTime(task.updatedAtMs || task.createdAtMs)}
                  </div>
                </div>
      ...
      <div className="rounded-2xl border bg-card p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Inspect task output or recent run history here.
        </p>
        <div className="mt-4 rounded-xl border bg-muted/20 p-3">
          <div className="mb-2 text-xs text-muted-foreground">
            {selectedTaskId ? `Task: ${selectedTaskId}` : 'No task selected'}
          </div>
          <pre className="max-h-[640px] overflow-auto whitespace-pre-wrap break-words text-xs leading-6">
            {runsText || '—'}
          </pre>
        </div>
      </div>
...
export function AgentDetails({ agentId }: { agentId: string }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const agentData = (await agentRes
          .json()
        ...
        const docResults = await Promise.all(
          docKeys.map(async (docKey) => {
            const res = await fetch(
              `/api/agents/${encodeURIComponent(agentId)}/docs/${docKey}`,
              { cache: 'no-store' }
            );
            ...
            return [
              docKey,
              data.ok === true ? data.data?.content || '' : '',
            ] as const;
          })
        );
  ...
  async function saveDoc() {
    ...
    try {
      const res = await fetch(
        `/api/agents/${encodeURIComponent(agentId)}/docs/${activeDocTab}`,
        {
          method: 'PUT',
          headers: { 'content-type': 'application/json' },
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="rounded-2xl border bg-card p-6 shadow-sm">
          <div className="mb-4 flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Manage AGENTS.md, IDENTITY.md, USER.md, SOUL.md, TOOLS.md.
              </p>
            </div>
            {!editingDoc ? (
              <button
        ...
        <div className="flex flex-col gap-6">
          <div className="rounded-2xl border bg-card p-6 shadow-sm">
            ...
            {basicsError && (
              <p className="mb-3 text-xs text-red-500">{basicsError}</p>
            )}
            {editingBasics ? (
              <div className="space-y-3 text-sm">
                <div>
                  ...
                  <input
                    value={basicsDraft.role}
                    onChange={(e) =>
                      setBasicsDraft((d) => ({ ...d, role: e.target.value }))
                    }
                    className="mt-1 w-full rounded-xl border bg-muted/30 px-3 py-2 text-sm outline-none"
                ...
                <div>
                  ...
                  <input
                    value={basicsDraft.model}
                    onChange={(e) =>
                      setBasicsDraft((d) => ({ ...d, model: e.target.value }))
                    }
                    className="mt-1 w-full rounded-xl border bg-muted/30 px-3 py-2 text-sm outline-none"
              ...
              <dl className="space-y-3 text-sm">
                ...
                <div>
                  ...
                  <dd className="mt-1 break-all font-medium">
                    {agent.enabled === false ? 'No' : 'Yes'}
                  </dd>
                </div>
                <div>
                  <dt className="text-muted-foreground">Model</dt>
                  <dd className="mt-1 break-all font-medium">
                    {agent.model || 'Use default'}
                  </dd>
                </div>
                <div>
          ...
          <div className="rounded-2xl border bg-card p-6 shadow-sm">
            ...
            <p className="mt-4 text-xs text-muted-foreground">
              Editing Telegram settings will be the next step. This page is
              currently read-only for validation.
            </p>
          </div>
        </div>
      </div>

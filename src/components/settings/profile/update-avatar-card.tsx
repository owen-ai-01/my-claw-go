...
export function UpdateAvatarCard({ className }: UpdateAvatarCardProps) {
  ...
  // Initialize avatarUrl directly from session to prevent hydration mismatch
  // This ensures SSR and client initial render have the same value
  const [avatarUrl, setAvatarUrl] = useState(session?.user?.image ?? '');
  const [tempAvatarUrl, setTempAvatarUrl] = useState('');

  useEffect(() => {
    // Update avatarUrl when session changes

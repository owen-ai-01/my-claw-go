import {
  findTelegramBotRoute,
  upsertTelegramChannelBinding,
} from '@/lib/myclawgo/telegram-routing';
...
export async function POST(
  ...
  if (!route?.bot || !route?.agent) {
    return NextResponse.json(
      { ok: false, error: 'Telegram bot route not found' },
      { status: 404 }
    );
  }
    ...
    return NextResponse.json(
      {
        ok: false,
        error:
          'Webhook secret not configured. Please re-register your Telegram bot.',
      },
      { status: 401 }
    );
  }

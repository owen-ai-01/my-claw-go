...
export const createCreditCheckoutSession = userActionClient
  ...
  .action(async ({ parsedInput, ctx }) => {
    ...
    try {
      ...
      console.log('Calling createCreditCheckout with params:', params);
      const result = await createCreditCheckout(params);
      console.log('createCreditCheckout result:', result);
      ...
      console.error('Create credit checkout session error (caught):', error);
      return {
        success: false,
        error:

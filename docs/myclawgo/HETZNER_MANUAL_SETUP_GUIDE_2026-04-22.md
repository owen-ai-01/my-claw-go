...
> **为什么 Bridge 不在 Snapshot 里**：Bridge 代码会频繁更新。新 VPS 注册回调后由 Control Plane（SaaS VPS）直接 SCP 推送最新构建产物；已运行的 VPS 通过批量更新脚本推送，整个过程无需 R2，无需重做 Snapshot。

### Snapshot 只需要重做的情况

| 变化 | 需要重做 Snapshot？ |
|------|------------------|
| Bridge 代码更新 | **不需要**（Control Plane SCP 推送） |
| OpenClaw 新版本 | 需要 |
| Node.js 大版本升级 | 需要 |
| systemd 服务文件变更 | 需要 |
...
> ⚠️ **必须指定版本号**：`npm install -g openclaw`（不加版本）会安装到 npm 上的无关占位包 `0.0.1`，`openclaw` 命令不可用。必须写 `@版本号`。

```bash
# 安装指定版本（root 全局，二进制自动放到 /usr/bin/openclaw）
npm install -g openclaw@2026.4.11

# 验证二进制可用
openclaw --version
# 应输出类似：openclaw/2026.4.11 linux-x64 node-v20.x.x
```

### 5.5 创建 openclaw 系统用户
...
# 验证 openclaw 用户也能访问 openclaw 命令（systemd 以此用户运行服务）
...
```bash
# Bridge 目录（内容由注册回调后 Control Plane SCP 填充，此处为空）
...
systemctl status myclawgo-bridge.service
# 显示 "Loaded: loaded ... disabled" 即正确，不需要 active
```

> **不要** `enable` 或 `start` 服务，cloud-init 会在新 VPS 启动时负责这一步。

### 5.7 清理模板机（制作 Snapshot 前必做）
...
```bash
# 在 SaaS 主机上执行
curl -s -H "Authorization: Bearer <步骤1的API_TOKEN>" \
  "https://api.hetzner.cloud/v1/images?type=snapshot" | python3 -m json.tool
# 找到 name 为 myclawgo-runtime-v1-20260426 的条目，记录 id 字段
```
...
Bridge 代码存在 SaaS VPS 项目目录，直接 SCP 推送到用户 VPS，**无需 R2**。
...
- **新创建的用户 VPS**：注册回调时 Control Plane 自动 SCP 推送最新构建产物
- **已运行的用户 VPS**：执行下面的批量更新脚本
...
**先生成注册 Token 密钥**（在 SaaS 主机上运行一次，复制输出）：

```bash
openssl rand -hex 32
# 输出示例：a3f8c2d1e4b7209f3c8a1d6e5f2b4c7d9e0a1b2c3d4e5f6a7b8c9d0e1f2a3b4
```

然后在 SaaS 主机的 `.env` 中追加：
...
# ────────────────────────────────────────────────────────────────────────
CONTROL_PLANE_PUBLIC_IP=<步骤 4 确认的 IP，如 46.225.210.174>

# ────────────────────────────────────────────────────────────────────────
# 注册 Token 签名密钥（cloud-init 回调鉴权用）
# 用 openssl rand -hex 32 生成，填入原始输出值
# ────────────────────────────────────────────────────────────────────────
RUNTIME_REGISTER_TOKEN_SECRET=<openssl rand -hex 32 的输出>

...
> **注意**：`npm install -g openclaw`（不指定版本）会装到 npm `latest` tag 对应的旧占位包 `0.0.1`，必须指定版本号。

```bash
# 指定版本安装（root 全局，二进制在 /usr/bin/openclaw）
...
# 确认 openclaw 用户也能访问（systemd 服务以此用户运行）
su - openclaw -c "openclaw --version"
```

> npm prefix 在此 Ubuntu 系统为 `/usr`，二进制装到 `/usr/bin/openclaw`。systemd 服务 `User=openclaw` 控制进程运行身份，不影响二进制安装位置。

### 5.5 创建 systemd 服务文件

```bash
# 创建目录（Bridge 内容由注册回调后 Control Plane SCP 填充）
...
# 不要 enable/start，cloud-init 负责启动
```
